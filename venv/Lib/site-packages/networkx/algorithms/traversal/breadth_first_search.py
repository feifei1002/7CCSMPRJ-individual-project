...
def bfs_edges(G, source, reverse=False, depth_limit=None, sort_neighbors=None):
    ...
    to already explored nodes. That means ``edge_bfs`` report all edges while
    ``bfs_edges`` only report those traversed by a node-based BFS. Yet another
    description is that ``bfs_edges`` report the edges traversed during BFS
    while ``edge_bfs`` report all edges in the order they are explored.
